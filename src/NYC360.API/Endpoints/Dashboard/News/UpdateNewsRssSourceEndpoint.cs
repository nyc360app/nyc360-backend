...
public class UpdateNewsRssSourceEndpoint(
    IMediator mediator,
    INewsAuthorizationService newsAuthorizationService,
    IRssSourceRepository rssSourceRepository)
    : Endpoint<UpdateNewsRssSourceRequest, StandardResponse>
{
    public override void Configure()
    {
        Put("/news-dashboard/rss/update");
        AllowFileUploads();
    }

    public override async Task HandleAsync(UpdateNewsRssSourceRequest req, CancellationToken ct)
    {
        ...
        var source = await rssSourceRepository.GetByIdAsync(req.Id, ct);
